...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            InlineKeyboardButton('⛔️ ᴄʟɪᴄᴋ ʜᴇʀᴇ ғᴏʀ ᴍᴏʀᴇ ʙᴜᴛᴛᴏɴs ⛔️', callback_data='help')
            ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        buttons = [[
            InlineKeyboardButton('💢 Cʟɪᴄᴋ Hᴇʀᴇ Tᴏ Cʟᴏsᴇ Tʜɪs Bᴜᴛᴛᴏɴs 💢', callback_data='start'),
            ],[
            InlineKeyboardButton('👑 ᴏᴡɴᴇʀ', callback_data='about'),
            InlineKeyboardButton('👥 ɢʀᴏᴜᴘ', url='https://t.me/M0vieslinkz')
            ],[
            InlineKeyboardButton('🚸 ᴀʙᴏᴜᴛ ', callback_data='autofilter'),
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb and imdb.get('poster'):
        try:
            fux = await message.reply_photo(photo=imdb.get('poster'), caption=cap[:1024],
                                      reply_markup=InlineKeyboardMarkup(btn))
        except (MediaEmpty, PhotoInvalidDimensions, WebpageMediaEmpty):
            pic = imdb.get('poster')
            poster = pic.replace('.jpg', "._V1_UX360.jpg")
            fux = await message.reply_photo(photo=poster, caption=cap[:1024], reply_markup=InlineKeyboardMarkup(btn))
        except Exception as e:
            logger.exception(e)
            fux = await message.reply_photo(photo=NORGE_IMG, caption=cap, reply_markup=InlineKeyboardMarkup(btn))
    else:
        fux = await message.reply_photo(photo=NORGE_IMG, caption=cap, reply_markup=InlineKeyboardMarkup(btn))
